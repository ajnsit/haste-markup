...
module Haste.Markup.Renderer.String
    ( fromChoiceString
    , renderMarkup
    , renderHtml
    ) where
...
import Haste.Prim (JSString, fromJSStr, toJSStr)

import Haste.Markup.Internal
...
-- AJ: TODO: Convert to use native JS foldr
tFoldr :: (Char -> String -> String) -> String -> JSString -> String
tFoldr op k s = foldr op k $ fromJSStr s

tIsInfixOf :: JSString -> JSString -> Bool
tIsInfixOf k s = fromJSStr k `isInfixOf` fromJSStr s
...
fromChoiceString (Text s)       = escapeMarkupEntities $ fromJSStr s
...
fromChoiceString (External x) = case x of
    ...
    Text   s     -> if "</" `tIsInfixOf` s then id else (\k -> tFoldr (:) k s)
    s            -> fromChoiceString s
